...
class ConfigCompanyTagModel extends Model
{
    ...
    {
        $aResult = $this->where(['delyn' => 'N'])->orderBy('idx', 'desc')->findAll();
        if ($aResult) {
            return $aResult;
        }
        return $aResult = [];
    ...
    public function getCompanyTag(): object
    {
        $aRow = $this
            ->where('delyn', 'N')
            ->orderBy('idx', 'random');
    ...
    public function getRanTagList(int $limit): array
    {
        $aResult = $this
            ->where(['delyn' => 'N'])
            ->orderBy('idx', 'RANDOM')
