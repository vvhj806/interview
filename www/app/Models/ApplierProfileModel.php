...
class ApplierProfileModel extends Model
{
    ...
    public function getAllprofile(int $memIdx, int $applyIdx)
    {
        if (!$memIdx && !$applyIdx) {
            $aAllprofile = "";
            return $aAllprofile;
        ...
        $aAllprofile = $this
            ...
            ->join('iv_file', 'iv_applier_profile.file_idx=iv_file.idx', 'left')
            ->where([
                'iv_applier_profile.mem_idx' => $memIdx,
                'iv_applier_profile.app_idx' => $applyIdx
            ])
            ->orderBy('iv_applier_profile.idx', 'desc');

        return $aAllprofile;
    }

    public function getMemberProfile(int $memIdx)
    {
        $aRow = "";
        if (!$memIdx) {
            return $aRow;
