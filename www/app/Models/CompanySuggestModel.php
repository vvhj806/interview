...
class CompanySuggestModel extends Model
{
    ...
    public function getSuggestInfo(int $interIdx): object
    {
        $aRow = [];
        if (!$interIdx) {
            return $aRow;
        ...
        $aRow = $this
                ->select('iv_company_suggest.com_idx, iv_company_suggest.sug_manager_name, iv_company_suggest.sug_manager_tel, iv_company_suggest.sug_start_date, iv_company_suggest.sug_end_date, iv_company_suggest.sug_start_time, iv_company_suggest.sug_end_time, iv_company.com_name')
                ->join('iv_company', 'iv_company.idx = iv_company_suggest.com_idx')
                ->where([
                    'iv_company_suggest.idx' => $interIdx
                ]);
