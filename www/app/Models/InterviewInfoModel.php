...
class InterviewInfoModel extends Model
{
    ...
    protected $createdField = 'info_reg_date'; // 데이터 레코드 작성 타임스탬프를 유지하기 위해 사용하는 데이터베이스 필드
    protected $updatedField = 'info_mod_date'; // 데이터 레코드 업데이트 타임스탬프를 유지하기 위해 사용할 데이터베이스 필드
    protected $deletedField = 'info_del_date'; // 데이터 레코드 삭제 타임스탬프를 유지하기 위해 사용할 데이터베이스 필드
        ...
        if (is_array($this->fields)) {
            foreach ($this->fields as $key => $row) {
                if (isset($row['auto_increment']) && $row['auto_increment'] === true) {
                    $this->primaryKey = $key;
                } else if (!in_array($key, ['info_reg_date', 'info_mod_date', 'info_del_date'])) {
    ...
    public function getComMock($comIdx)
    {
        $aRow = [];
        if (!$comIdx) {
            return $aRow;
        ...
        $aRow = $this
            ->select('iv_interview.idx, iv_interview.inter_name, iv_interview.inter_question, iv_job_category.job_depth_text')
            ->join('iv_interview', 'iv_interview.idx = iv_interview_info.inter_idx', 'left')
            ->join('iv_job_category', 'iv_job_category.idx = iv_interview.job_idx_position', 'left')
            ->where([
                'iv_interview_info.com_idx' => $comIdx,
                'iv_interview_info.delyn' => 'N',
                'iv_interview.inter_type' => 'B',
