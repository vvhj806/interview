...
class CompanyTagModel extends Model
{
    ...
    public function getCompanyTag($comIdx): array
    {
        $aRow = [];
        if (!$comIdx) {
            return $aRow;
        ...
        $aRow = $this
            ...
            ->join('config_company_tag', 'iv_company_tag.config_tag_idx = config_company_tag.idx')
            ->where([
                'iv_company_tag.com_idx' => $comIdx,
                'iv_company_tag.delyn' => 'N',
                'config_company_tag.delyn' => 'N'
            ])
            ->findAll();        
    ...
    public function getCompanyTagIdx($comIdx): array
    {
        $aRow = [];
        if (!$comIdx) {
            return $aRow;
        ...
        $aRow = $this
            ...
            ->findColumn('config_tag_idx');        

        return $aRow ?? [];
    }

    public function getCompanyTagCount($comIdx): int
    {
        $iRow = 0;
        if (!$comIdx) {
            return $iRow;
