...
class ReportResultModel extends Model
{
    ...
    public function getMaxScoreInterview($applierIdx): int
    {
        ...
        $aMaxScoreInterview = $this
            ->join('iv_applier', 'iv_applier.idx = iv_report_result.applier_idx', 'inner')
            ->where([
                'iv_report_result.que_type' => 'T',
                'iv_applier.app_type' => 'M'
            ])
    ...
    public function getReportInfo(int $applierIdx): array
    {
        ...
        if ($aReportInfoGrade['repo_analysis'] == null || $aReportInfoGrade['repo_analysis'] == "") {
            $aReportInfoGrade['repo_analysis'] = array('sum' => 0, 'grade' => '-', 'activity' => 0, 'stability' => 0, 'willpower' => 0, 'attraction' => 0, 'affirmative' => 0, 'reliability' => 0, 'aggressiveness' => 0);
        }
    ...
    public function getAllgetfacialAnalysis()
    {
        $aRow = [];

        $aRow = $this
            ->select('iv_report_result.repo_score')
            ->join('iv_applier', 'iv_applier.idx = iv_report_result.applier_idx')
            ->where([
                'iv_applier.app_iv_stat' => 4,
                'iv_applier.delyn' => 'N',
                'iv_report_result.repo_score IS NOT' => null,
                'iv_report_result.que_type' => 'T',
                'iv_report_result.repo_process' => 2
                ])
            ->findAll();
