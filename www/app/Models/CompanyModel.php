...
class CompanyModel extends Model
{
    ...
    public function getPracticeList(string $strSearchText = null, array $aSearchComForm = null): object
    {
        ...
        $this
            ...
            ->where(['iv_company.com_practice_interview' => 'Y', 'iv_company.delyn' => 'N',]); //모의면접 허용
        ...
        if ($aSearchComForm) { //회사형태에 대한 검색어가 있으면
            $aOrWhere = [];
            for ($i = 0, $max = count($aSearchComForm); $i < $max; $i++) {
                $aOrWhere[] = $aSearchComForm[$i];
            }
            $this->whereIn('iv_company.com_form', $aOrWhere);
        }

        $aRow = $this
            ->orderBy('iv_company.com_score', 'DESC');

        return $aRow;
    }

    public function getTagList(array $aTagIdx = [], string $innerOrLeft): object
    {
        $readyDB = $this->select([
            ...
            ->join('iv_recruit', 'iv_recruit.com_idx = iv_company.idx', $innerOrLeft)
            ->join('iv_file', 'iv_file.idx = iv_company.file_idx_logo', 'left');
    ...
    public function getAllCompany(): object
    {
        $today = date('Ymd');
        $aRow = $this
            ->select('iv_company.idx, iv_company.com_name, iv_company.com_industry, iv_company.com_address, iv_file.file_save_name')
    ...
    public function getComInfo(array $comIdx): array
    {
        $aRow = [];
        if (!$comIdx) {
            return $aRow;
        ...
        $aComInfo = [];
        foreach ($comIdx as $key => $val) {
            $aRow = $this
                ->select('iv_company.idx, iv_company.com_name, iv_company.com_address, COUNT(iv_recruit.idx) as recCnt, iv_file.file_save_name')
            ...
            array_push($aComInfo, $aRow);
        }

        return $aComInfo ?? [];
    }

    public function exploreComCnt(): array
    {
        $aRow = [];
        $aRow = $this
            ->select('COUNT(idx) AS comCnt', '', false)
            ->where([
                'com_practice_interview' => 'Y',
                'delyn' => 'N'
            ])
            ->first();
