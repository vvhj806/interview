...
class CompnaySuggestModel extends Model
{
    ...
    public function getSuggestList(int $iMemberIdx): object
    {
        ...
        $aResult = $this
            ->select([
                'iv_company_suggest.idx', 'rec_end_date', 'sug_end_date',
                'sug_type', 'sug_idx', 'com_name', 'rec_title',
            ])
            ...
            ->where([
                'config_company_suggest.mem_idx' => $iMemberIdx, 'iv_company_suggest.delyn' => 'N',
                'iv_company.delyn' => 'N', 'config_company_suggest.delyn' => 'N',
                'iv_applier.delyn' => 'N', 'iv_recruit.rec_stat' => 'Y'
            ]);
        return $aResult;
    }
    ...
    public function getSuggestDetail(int $iMemberIdx, int $iSuggestIdx): object
    {
        ...
        $aResult = $this
            ->select([
                'iv_company_suggest.idx', 'iv_company.idx as comIdx', 'sug_type', 'rec_end_date',
                'sug_end_date', 'sug_manager', 'com_name', 'rec_title',
                'sug_massage', 'sug_manager_name', 'sug_manager_tel', 'iv_recruit.idx as recIdx'
            ])
            ->join('iv_company', 'iv_company.idx = iv_company_suggest.com_idx', 'inner')
            ->join('config_company_suggest', 'config_company_suggest.sug_idx = iv_company_suggest.idx', 'inner')
            ->join('iv_applier', 'iv_applier.idx = iv_company_suggest.app_idx', 'inner')
            ->join('iv_recruit', "iv_recruit.idx = iv_applier.rec_idx AND iv_recruit.delyn = 'N'", 'left')
            ->where([
                'iv_company_suggest.idx' => $iSuggestIdx, 'config_company_suggest.mem_idx' => $iMemberIdx,
                'iv_company_suggest.delyn' => 'N', 'iv_company.delyn' => 'N',
                'config_company_suggest.delyn' => 'N', 'iv_applier.delyn' => 'N', 'iv_recruit.stat' => 'Y'
            ]);
        return $aResult;
    }

    public function configSuggestBuilder(): object
    {
        $objQuery = $this
            ->join(
                'config_company_suggest',
                'iv_company_suggest.idx = config_company_suggest.sug_idx AND config_company_suggest.delyn = "N"',
                'left'
            )
    ...
    public function applicantSuggestBuilder(): object
    {
        $objQuery = $this
            ->join(
                'iv_company_suggest_applicant',
                'iv_company_suggest.idx = iv_company_suggest_applicant.sug_idx AND iv_company_suggest_applicant.delyn = "N"',
                'left'
            );
        return $objQuery;
    }
    ...
    {
        $objQuery = $this
            ->join('iv_company', 'iv_company_suggest.com_idx = iv_company.idx', 'left');
        return $objQuery;
    }
    ...
    public function daysInBullder(int $iDays = 1) // 오늘 - iDays의 값만
    {
        ...
        $objQuery = $this
            ->where(['iv_company_suggest.sug_reg_date <=' => $today, 'iv_company_suggest.sug_reg_date >=' => $beforeDay]);
        return $objQuery;
    }
    ...
    public function getMyData(int $iMemIdx)
    {
        ...
        $objQuery = $this
            ->configSuggestBuilder()
            ->where(['config_company_suggest.mem_idx' => $iMemIdx]);
        return $objQuery;
    }

    public function checkSuggest(int $iMemIdx, int $iDays): bool
    {
        $boolResult = $this->select(['config_company_suggest.idx'])->daysInBullder($iDays)->getMyData($iMemIdx)->first();
        return $boolResult ? true : false;
    }

    public function selectorAdminWrite(): object
    {
        $objQuery = (object)[];
    ...
    public function getSuggestTypeMsg($sugTypeValue): string
    {
        $strMsg = '';
        if ($sugTypeValue === 'A') {
            $strMsg = '인터뷰제안';
        } elseif ($sugTypeValue === 'I') {
            $strMsg = '대면면접제안';
        } elseif ($sugTypeValue === 'O') {
            $strMsg = '이직및포지션제안';
        }
