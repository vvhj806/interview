...
class SendLib
{
    ...
    public function telegramSend(string $msg, string $botType = null)
    {
        ...
        $globalvar = new GlobalvarLib();
        if (in_array($globalvar->getServerHost(), ['test', 'webtest'])) {
            //test
            $strCurl = "https://api.telegram.org/bot1626097026:AAHsx6T0sUYtZtJyGfb4XA-_le8BKqCauxA/sendmessage?chat_id=-602426796&parse_mode=HTML&text="; //테스트용 텔레그램은 한방에 다 보내기
        } else {
            //real
            if ($botType == "HB") {
